...
### 方案零：本机一键运行（推荐本地/公司电脑使用，无需 Docker 和 sudo）

```bash
./run.sh              # 装依赖(如缺) → 构建(如缺) → 启动，访问 http://localhost:3001
./run.sh --rebuild    # 强制重新构建后启动
./run.sh --check      # 冒烟自检（临时端口 + 内存库，不碰真实数据），CI 可用
./run.sh --dev        # 开发模式（前后端热更新）
```

- 前置要求仅一个：Node.js >= 20.19
- 配置走环境变量或仓库根 `.env`（自动加载）；不配置也能跑（SQLite 默认落在 `~/.config/efficio/`）
- 依赖安装失败会自动换 npmmirror 镜像重试（国内网络友好）
